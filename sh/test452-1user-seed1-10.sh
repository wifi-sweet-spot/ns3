...
INIT_FILE_NAME="test452-1user-seed1-10"

INIT_NUM_TCP_USERS=1
STEP_NUM_TCP_USERS=2
UPPER_LIMIT_NUM_TCP_USERS=1
...
for ((i=INIT_NUM_TCP_USERS; i<=UPPER_LIMIT_NUM_TCP_USERS; i=$((${i}+${STEP_NUM_TCP_USERS})) )); do
  ...
  for ((seed=INITSEED; seed<=MAXSEED; seed++)); do
	...
	executablename_string=${executablename_string}"scratch/wifi-central-controlled-aggregation_v199"

	parameters_string=${parameters_string}" --simulationTime=180 \
	...
	#--methodAdjustAmpdu=2 --stepAdjustAmpdu=3000 "
