...
INIT_FILE_NAME="test434"

INIT_NUM_TCP_USERS=2
STEP_NUM_TCP_USERS=2
...
for ((i=INIT_NUM_TCP_USERS; i<=UPPER_LIMIT_NUM_TCP_USERS; i=$((${i}+${STEP_NUM_TCP_USERS})) )); do
  ...
  for ((seed=INITSEED; seed<=MAXSEED; seed++)); do
    ...
    executablename_string=${executablename_string}"scratch/wifi-central-controlled-aggregation_v215"
    ...
    parameters_string=${parameters_string}" \
        ...
        --writeMobility=1 \
        --numOperationalChannels=16 \
        --numOperationalChannelsSecondary=16 \
        --verboseLevel=3 \
        --printSeconds=10 \
        --channelWidth=20 \
        ...
        --RtsCtsThreshold=0 \
        --defineAPsManually=0 \
        --defineSTAsManually=0 \
        --numberAPsSamePlace=1 \
        --numberWiFiDevicesInSTAs=1 \
    ...
    parameters_string=${parameters_string}"--rateAPsWithAMPDUenabled=1.0 --aggregationDisableAlgorithm=0 \
       ...
       --methodAdjustAmpdu=5 --stepAdjustAmpdu=3000 "
